...
g = 3
log = [0xff, 0x00, 0x19, 0x01, 0x32, 0x02, 0x1a,
    ...
    0xc0, 0xf7, 0x70, 0x07]
exp = [0x01, 0x03, 0x05, 0x0f, 0x11, 0x33, 0x55,
...
def div(a,b: int):
    inverse = exp[255-log[b]]
    return mul(a, inverse)

def split(s,n,k):
    num = 0
    if isinstance(s, type('a')):
        num = ord(s)
    elif isinstance(s, int) or isinstance(s, np.uint8):
        num = s
    a_list = np.random.randint(0, 256, k-1,dtype=np.uint8)
    b_list = [None]*(k-1)
    #np.random.seed(seed_num)
    numbers = np.random.choice(range(256),size=n,replace=False)
    result = []
    for i in range(n):
        b_list[k-2] = a_list[k-2]
        for j in range(k-3,-1,-1):
            b_list[j] = add(a_list[j],mul(b_list[j+1],numbers[i]))
        share = add(num,mul(numbers[i],b_list[0]))
        result.append((numbers[i],share))
    return result
        
def combine(k: int, r: list):
    if (len(r) != k):
        return None
    result = 0
    for i in range(k):
        ...
        for j in range(k):
            if (i != j):
                xi = r[i][0]
                xj = r[j][0]
                product = mul(product, div(sub(0, xj),sub(xi,xj)))
        result = add(result, mul(product, r[i][1]))
    return result

#k = 3
#n = 5
#secret = 'a'
#r = split(secret, n, k) # a = 97
#print("secret:",ord(secret))
#print("split: ", r)
#r = [[196, 27], [123, 135], [165, 85], [131, 204], [111, 164]]
#print("shares used to reconstruct secret:",r[0:k])
#print("combine: ", combine(3, r[0:3]))
